...
const TeacherProfileModalContent = ({ teacherProfile }) => {
  const { pendingentrymessage, teacherprofileId } = teacherProfile;
  ...
  const { remuneration, daysAvailable, username, category, subcategory } =
    Object.keys(teacherProfile).length === 0 ? "" : teacherprofileId;
  return (
    <div>
      ...
      <p className="mb-2">
        <strong>Entry Message:</strong> {pendingentrymessage}
      </p>
    </div>
  );
};
...
const TeacherListPending = () => {
  const { PendingRequestSent } = useSelector((state) =>
    state.user && state.user.user ? state.user.user : { PendingRequestSent: [] }
  );
  ...
  const totalPages = Math.ceil(PendingRequestSent.length / itemsPerPage);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = PendingRequestSent.slice(
    indexOfFirstItem,
    Math.min(indexOfLastItem, PendingRequestSent.length) // Adjust here
  );
...
export default TeacherListPending;
